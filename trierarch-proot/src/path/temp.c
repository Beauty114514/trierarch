...
static int clean_temp_cwd()
{
	...
	while (1) {
		...
		if (status < 0) {
#ifdef __ANDROID__
			/* On Android app storage chmod often returns ENOSYS; try chdir anyway. */
			(void) status;
#else
...
static int remove_temp_directory2(const char *path)
{
	...
	if (status < 0) {
#ifdef __ANDROID__
		/* On Android app storage chmod often returns ENOSYS; app owns the dir, try chdir anyway. */
		(void) status;
#else
	...
	if (cwd != NULL) {
		...
		if (status < 0) {
#ifdef __ANDROID__
			/* On Android, chdir("/") from app context can fail (ENOSYS). Use temp dir as fallback. */
			status = chdir(get_temp_directory());
			if (status < 0)
				(void) status; /* best-effort; process may be exiting */
#else
			result = -1;
