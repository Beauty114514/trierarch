...
"To workaround it, set the env. variable PROOT_NO_SECCOMP to 1.");
	...
	/* Android: execve from app context often returns ENOSYS (e.g. seccomp or ptrace restrictions). */
	if (status == -ENOSYS) {
		note(tracee, INFO, USER,
"On Android, execve can fail with \"Function not implemented\" when run from an app (vs adb shell).\n"
"Set PROOT_NO_SECCOMP=1 in the app environment to disable seccomp in the tracee; this often fixes it.");
		return;
	}
#endif
