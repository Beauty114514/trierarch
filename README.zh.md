...
- 在侧栏开启 **Wayland**，切换到 Wayland 视图即可看到合成器画面。触摸作为指针（指针模式见下文 **View Settings**）；点击 **Keyboard** 将按键发给当前焦点 client。
...
在侧栏或相关入口打开 **View Settings**，可调整合成器画面与指针行为。界面示意：

![View Settings](docs/images/viewSettings.jpg)

- **指针 / 鼠标模式**：提供两种模式，例如 **触摸板（相对）** 与 **平板（绝对）**，对应不同的移动方式与操作习惯（与 Wayland 视图中的触摸指针一致）。
- **界面元素大小与清晰度**：有两类独立控制，可组合使用：
  - **Resolution（分辨率）模式**：降低输出分辨率，从而改变界面元素在屏幕上的视觉大小，并**减轻** GPU / 合成负载，适合更看重流畅或省电的场景。
  - **Scale（缩放）模式**：**不改变**底层分辨率，通过**缩放**调整界面元素大小，**尽量保持**文字与控件的清晰锐利。
- **组合**：两种模式可**叠加**；按设备性能与个人观感，在「性能」与「清晰度」之间自行搭配，找到最适合自己的组合。
