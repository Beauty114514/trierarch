...
public class MainActivity extends AppCompatActivity {
    ...
    public static MainActivity getInstance() {
        if (instance != null)
            return instance;
        android.app.Activity h = X11ActivityBridge.getHostActivity();
        return (h instanceof MainActivity) ? (MainActivity) h : null;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        app.trierarch.X11Runtime.ensureX11ServerProcessStarted(this);

        prefs = new Prefs(this);
        int modeValue;
        try {
            modeValue = Integer.parseInt(prefs.touchMode.get()) - 1;
        } catch (Exception e) {
            modeValue = 0;
        ...
        setContentView(R.layout.termux_main_activity);
        ...
        }}, SDK_INT >= VERSION_CODES.TIRAMISU ? Context.RECEIVER_NOT_EXPORTED : 0);
    ...
    protected void onStart() {
        super.onStart();
        app.trierarch.X11Runtime.INSTANCE.setLorieInputRouteTopVisible(true);
    }

    @Override
    protected void onStop() {
        app.trierarch.X11Runtime.INSTANCE.setLorieInputRouteTopVisible(false);
        super.onStop();
    }
    ...
    protected void onDestroy() {
        ...
        try { com.termux.x11.LorieView.connect(-1); } catch (Throwable __) {}
        X11ActivityBridge.setLorieView(null);
        if (X11ActivityBridge.getHostActivity() == this)
    ...
    void onPreferencesChangedCallback() {
        ...
        if (SDK_INT >= VERSION_CODES.JELLY_BEAN_MR1 && isDestroyed()) {
            return;
        }
        prefs.recheckStoringSecondaryDisplayPreferences();
        ...
        if (lorieView == null) {
            // Host may be embedded in Trierarch: our LorieView is not in this content view.
            return;
        }
    ...
    public static void getRealMetrics(DisplayMetrics m) {
        LorieView lv = X11ActivityBridge.getLorieView();
        if (lv != null && lv.getDisplay() != null) {
            lv.getDisplay().getRealMetrics(m);
            return;
        }
        MainActivity inst = getInstance();
        if (inst != null && inst.getLorieView() != null && inst.getLorieView().getDisplay() != null)
            inst.getLorieView().getDisplay().getRealMetrics(m);
    }

    /** Called when the host is Compose (no Termux {@link MainActivity} instance). */
    public static void notifyExternalKeyboardConnected(boolean connected) {
        externalKeyboardConnected = connected;
        MainActivity inst = getInstance();
        if (inst != null)
            inst.setExternalKeyboardConnected(connected);
    }
