...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TRIERARCH_WINLATOR_LOG, "onCreate: rootFS=" + rootFS + " generate_wineprefix=" + isGenerateWineprefix()
            + " container_id=" + getIntent().getIntExtra("container_id", 0));

        if (!isGenerateWineprefix()) {
            ContainerManager containerManager = new ContainerManager(this);
            container = containerManager.getContainerById(getIntent().getIntExtra("container_id", 0));
            if (container == null) {
                Log.w(TRIERARCH_WINLATOR_LOG, "onCreate: container is null, finishing");
                finish();
                return;
            }
            containerManager.activateContainer(container);
            Log.i(TRIERARCH_WINLATOR_LOG, "onCreate: activated container id=" + container.id + " rootDir=" + container.getRootDir());

            boolean wineprefixNeedsUpdate = container.getExtra("wineprefixNeedsUpdate").equals("t");
            if (wineprefixNeedsUpdate) {
                Log.i(TRIERARCH_WINLATOR_LOG, "onCreate: wineprefixNeedsUpdate=true, running updateWineprefix");
                preloaderDialog.show(R.string.updating_system_files);
                WineUtils.updateWineprefix(this, (status) -> {
                    Log.i(TRIERARCH_WINLATOR_LOG, "updateWineprefix callback status=" + status);
                    if (status == 0) {
        ...
        Log.i(TRIERARCH_WINLATOR_LOG, "onCreate: showing starting_up preloader");
        ...
        xServer.windowManager.addOnWindowModificationListener(new WindowManager.OnWindowModificationListener() {
            ...
            public void onMapWindow(Window window) {
                Log.d(TRIERARCH_WINLATOR_LOG, "onMapWindow: id=" + window.id + " renderable=" + window.isRenderable()
                    + " className=\"" + window.getClassName() + "\" alreadyDismissedPreloader=" + flags[0]);
                if (!flags[0] && window.isRenderable() && !window.getClassName().isEmpty()) {
                    Log.i(TRIERARCH_WINLATOR_LOG, "onMapWindow: closing preloader (first qualifying window)");
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            Log.i(TRIERARCH_WINLATOR_LOG, "bg: startup thread begin");
            try {
                if (!isGenerateWineprefix()) {
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: setupWineSystemFiles begin");
                    setupWineSystemFiles();
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: setupWineSystemFiles end");
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: extractGraphicsDriverFiles begin");
                    extractGraphicsDriverFiles();
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: extractGraphicsDriverFiles end");
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: changeWineAudioDriver begin");
                    changeWineAudioDriver();
                    Log.i(TRIERARCH_WINLATOR_LOG, "bg: changeWineAudioDriver end");
                }
                Log.i(TRIERARCH_WINLATOR_LOG, "bg: setupXEnvironment begin");
                setupXEnvironment();
                Log.i(TRIERARCH_WINLATOR_LOG, "bg: setupXEnvironment end (guest should be running)");
            }
            catch (Throwable t) {
                Log.e(TRIERARCH_WINLATOR_LOG, "bg: startup thread failed", t);
            }
        });
    }
    ...
    private void setupXEnvironment() {
        ...
        Log.i(TRIERARCH_WINLATOR_LOG, "setupXEnvironment: rootPath=" + rootPath + " WINEPREFIX will be " + rootPath + RootFS.WINEPREFIX);
        ...
        String ge = guestProgramLauncherComponent.getGuestExecutable();
        Log.i(TRIERARCH_WINLATOR_LOG, "setupXEnvironment: guestExecutable(final)=" + (ge == null ? "null" : trimLog(ge)));
        Log.i(TRIERARCH_WINLATOR_LOG, "setupXEnvironment: startEnvironmentComponents begin");
        environment.startEnvironmentComponents();
        Log.i(TRIERARCH_WINLATOR_LOG, "setupXEnvironment: startEnvironmentComponents returned");

        winHandler.start();
        Log.i(TRIERARCH_WINLATOR_LOG, "setupXEnvironment: winHandler.start done");
        envVars.clear();
        graphicsDriver = null;
    ...
    /** Keeps logcat lines readable (Log buffer limit ~4k per line on some devices). */
